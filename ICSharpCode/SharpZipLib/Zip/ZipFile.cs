...
namespace ICSharpCode.SharpZipLib.Zip
{
  public class ZipFile : IEnumerable, IDisposable
  {
    ...
    public ZipEntry GetEntry(string name)
    {
      ...
      return entry >= 0 ? (ZipEntry) this.entries_[entry].Clone() : (ZipEntry) null;
    }
    ...
    private long TestLocalHeader(ZipEntry entry, ZipFile.HeaderTest tests)
    {
      ...
      {
        bool flag1 = (uint) (tests & ZipFile.HeaderTest.Header) > 0U;
        bool flag2 = (uint) (tests & ZipFile.HeaderTest.Extract) > 0U;
        ...
        if (flag1)
        {
          ...
          if (((uint) num2 & 64U) > 0U && num1 < (short) 62)
            throw new ZipException("Strong encryption flag set but version not high enough");
          if (((uint) num2 & 8192U) > 0U && (num4 != (short) 0 || (uint) num5 > 0U))
        ...
        if (((int) num2 & 8) == 0 || num8 > 0L || num7 > 0L)
        {
          ...
          if (num7 != entry.CompressedSize && num7 != (long) uint.MaxValue && num7 != -1L)
    ...
    private void WriteLocalEntryHeader(ZipFile.ZipUpdate update)
    {
      ...
      if ((uint) outEntry.ExtraData.Length <= 0U)
        return;
    ...
    {
      int num = 0;
      if ((uint) (update.Entry.Flags & 8) > 0U)
      {
        num = 12;
    ...
    {
      Stream stream1 = this.baseStream_;
      if (entry.IsCrypted)
        stream1 = this.CreateAndInitEncryptionStream(stream1, entry);
      Stream stream2;
      switch (entry.CompressionMethod)
      {
        case CompressionMethod.Stored:
          stream2 = (Stream) new ZipFile.UncompressedStream(stream1);
          break;
        case CompressionMethod.Deflated:
          stream2 = (Stream) new DeflaterOutputStream(stream1, new Deflater(9, true))
          {
            IsStreamOwner = false
          };
          break;
    ...
    private void ReadEntries()
    {
      ...
      if (num6 > 0U)
      {
        byte[] numArray = new byte[(int) num6];
      ...
      for (ulong index = 0; index < length1; ++index)
      {
        ...
        long num14 = (long) this.ReadLEUint();
        long num15 = (long) this.ReadLEUint();
        ...
        long num21 = (long) this.ReadLEUint();
        byte[] numArray = new byte[Math.Max(num16, num17)];
        StreamUtils.ReadFully(this.baseStream_, numArray, 0, num16);
        ZipEntry zipEntry = new ZipEntry(ZipConstants.ConvertToStringExt(flags, numArray, num16), versionRequiredToExtract, madeByInfo, (CompressionMethod) num11);
        zipEntry.Crc = (long) num13 & (long) uint.MaxValue;
        zipEntry.Size = num15 & (long) uint.MaxValue;
        zipEntry.CompressedSize = num14 & (long) uint.MaxValue;
        zipEntry.Flags = flags;
        zipEntry.DosTime = (long) num12;
        zipEntry.ZipFileIndex = (long) index;
        zipEntry.Offset = num21;
        zipEntry.ExternalFileAttributes = (int) num20;
        zipEntry.CryptoCheckValue = (flags & 8) != 0 ? (byte) (num12 >> 8 & (uint) byte.MaxValue) : (byte) (num13 >> 24);
        if (length2 > 0)
        {
        ...
        if (num17 > 0)
        {
          StreamUtils.ReadFully(this.baseStream_, numArray, 0, num17);
          zipEntry.Comment = ZipConstants.ConvertToStringExt(flags, numArray, num17);
        }
    ...
    private Stream CreateAndInitDecryptionStream(Stream baseStream, ZipEntry entry)
    {
      ...
      else
      {
        ...
        if (num != aesSaltLen)
          throw new ZipException("AES Salt expected " + (object) aesSaltLen + " got " + (object) num);
    ...
    private Stream CreateAndInitEncryptionStream(Stream baseStream, ZipEntry entry)
    {
      ...
      if (entry.Version < 50 || (entry.Flags & 64) == 0)
      {
        ...
        if (entry.Crc < 0L || (uint) (entry.Flags & 8) > 0U)
