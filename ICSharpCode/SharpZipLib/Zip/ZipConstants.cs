...
namespace ICSharpCode.SharpZipLib.Zip
{
  public sealed class ZipConstants
  {
    ...
    {
      get => ZipConstants.defaultCodePage != 1 ? ZipConstants.defaultCodePage : 437;
      set => ZipConstants.defaultCodePage = value;
    }

    public static string ConvertToString(byte[] data, int count) => data == null ? string.Empty : Encoding.GetEncoding(ZipConstants.DefaultCodePage).GetString(data, 0, count);
    ...
    public static string ConvertToStringExt(int flags, byte[] data, int count)
    {
      if (data == null)
        return string.Empty;
      return (uint) (flags & 2048) > 0U ? Encoding.UTF8.GetString(data, 0, count) : ZipConstants.ConvertToString(data, count);
    }
    ...
    public static string ConvertToStringExt(int flags, byte[] data)
    {
      if (data == null)
        return string.Empty;
      return (uint) (flags & 2048) > 0U ? Encoding.UTF8.GetString(data, 0, data.Length) : ZipConstants.ConvertToString(data, data.Length);
    }

    public static byte[] ConvertToArray(string str) => str == null ? new byte[0] : Encoding.GetEncoding(ZipConstants.DefaultCodePage).GetBytes(str);

    public static byte[] ConvertToArray(int flags, string str)
    {
      if (str == null)
        return new byte[0];
      return (uint) (flags & 2048) > 0U ? Encoding.UTF8.GetBytes(str) : ZipConstants.ConvertToArray(str);
    }
