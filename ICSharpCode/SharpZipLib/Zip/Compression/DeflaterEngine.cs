...
namespace ICSharpCode.SharpZipLib.Zip.Compression
{
  public class DeflaterEngine : DeflaterConstants
  {
    ...
    private bool FindLongestMatch(int curMatch)
    {
      ...
      while ((curMatch = (int) prev[curMatch & (int) short.MaxValue] & (int) ushort.MaxValue) > num2 && (uint) --maxChain > 0U);
    ...
    private bool DeflateStored(bool flush, bool finish)
    {
      ...
      if (((storedLength >= DeflaterConstants.MAX_BLOCK_SIZE ? 1 : (this.blockStart >= 32768 ? 0 : (storedLength >= 32506 ? 1 : 0))) | (flush ? 1 : 0)) == 0)
        return true;
