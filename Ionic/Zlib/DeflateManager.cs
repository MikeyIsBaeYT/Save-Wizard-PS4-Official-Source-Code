...
namespace Ionic.Zlib
{
  internal sealed class DeflateManager
  {
    ...
    internal int build_bl_tree()
    {
      ...
      while (index >= 3 && (uint) this.bl_tree[(int) Tree.bl_order[index] * 2 + 1] <= 0U)
        --index;
...
label_13:
      ...
      return this._codec.AvailableBytesOut == 0 ? (flush == FlushType.Finish ? BlockState.FinishStarted : BlockState.NeedMore) : (flush == FlushType.Finish ? BlockState.FinishDone : BlockState.BlockDone);
    }
    ...
    private void _fillWindow()
    {
      ...
      while (this.lookahead < DeflateManager.MIN_LOOKAHEAD && (uint) this._codec.AvailableBytesIn > 0U);
      goto label_12;
label_16:
      return;
label_12:;
      ...
      while (true)
      {
        ...
        if (this.match_length >= DeflateManager.MIN_MATCH)
        {
          ...
          if (this.match_length <= this.config.MaxLazy && this.lookahead >= DeflateManager.MIN_MATCH)
          {
            ...
            while ((uint) --this.match_length > 0U);
            ++this.strstart;
          }
          else
          {
    ...
    internal BlockState DeflateSlow(FlushType flush)
    {
      ...
      while (true)
      {
        ...
        if (this.prev_length >= DeflateManager.MIN_MATCH && this.match_length <= this.prev_length)
        {
          ...
          while ((uint) --this.prev_length > 0U);
          this.match_available = 0;
          this.match_length = DeflateManager.MIN_MATCH - 1;
          ++this.strstart;
          if (flag)
        ...
        else if ((uint) this.match_available > 0U)
        {
...
label_24:
      return BlockState.NeedMore;
label_28:
      if ((uint) this.match_available > 0U)
      {
    ...
    internal int longest_match(int cur_match)
    {
      ...
      do
      {
        ...
        if ((int) this.window[index2 + num1] == (int) num6 && (int) this.window[index2 + num1 - 1] == (int) num5 && (int) this.window[index2] == (int) this.window[index1] && (int) this.window[++index2] == (int) this.window[index1 + 1])
        {
          ...
          int num9 = DeflateManager.MAX_MATCH - (num4 - num7);
      ...
      while ((cur_match = (int) this.prev[cur_match & wMask] & (int) ushort.MaxValue) > num2 && (uint) --maxChainLength > 0U);
    ...
    internal int SetParams(CompressionLevel level, CompressionStrategy strategy)
    {
      ...
      if (this.compressionLevel != level)
      {
        ...
        if (config.Flavor != this.config.Flavor && (ulong) this._codec.TotalBytesIn > 0UL)
    ...
    internal int Deflate(FlushType flush)
    {
      ...
      if ((uint) this.pendingCount > 0U)
      {
        ...
        return 0;
      if (this.status == DeflateManager.FINISH_STATE && (uint) this._codec.AvailableBytesIn > 0U)
      {
        this._codec.Message = DeflateManager._ErrorMessage[7];
